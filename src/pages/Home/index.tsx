...
function Home() {
  ...
  const apikey =
    '6dccb4225f5fc86fb2fdb88c13fc04f0&hash=b84aa685a8d78ea4f0c68c6e4207ae94';
  const limit = 20;
  ...
  const loadComics = async () => {
    ...
    const response = await api.get(
      `comics?limit=${limit}&offset=${offset}&ts=${ts}&apikey=${apikey}`
    );
  ...
  const renderItem: ListRenderItem<Comic> = ({ item }) => (
    <CardHome
      ...
      markeds={markeds}
      onClick={() =>
        navigation.navigate({ name: 'Details' as never, params: item as never })
      }
    />
  );
