...
const CartProvider = ({children}:CartProviderProps ) => {
  ...
  const addToCart = useCallback(
    async (comic, comicQuantity ) => {
      ...
      const quantity = comicExists ? comicExists.quantity + comicQuantity : comicQuantity;

      if (comicExists) {
        setCart(
          cart.map(p => (p.id === comic.id ? { ...comic, quantity } : p)),
          );
        } else {
        setCart([...cart, { ...comic, quantity }]);
        }
        await AsyncStorage.setItem(
        '@MarvelStore_Cart',
        JSON.stringify([...cart, { ...comic, quantity }]),
        );
      },
    [cart, setCart],
  );
